...
| foreign-key field, non-null key        | `{ <field>: { <public-pk>: <id> } }` — strawberry-django models a relation filter as a nested input exposing its configured `DEFAULT_PK_FIELD_NAME` (default `pk`, an `ID` scalar), not `exact`. The lookup *name* always tracks `DEFAULT_PK_FIELD_NAME`; the lookup *value* defaults to the raw group key (`<field>_id`). That raw value is correct only when the configured lookup accepts a raw database pk (`pk`, `id`). If `DEFAULT_PK_FIELD_NAME` names an **opaque** scheme (`sqid`, `hashid`, …), the raw pk would re-select nothing, so `filter_echo_relation_identity` is **required**, not optional — see below. |
...
**Value form.** A clause value is what the **lookup's resolved type** accepts — resolved from the live `filter_type`, never assumed. A `choices` column is the load-bearing case because it can be filtered two ways:

- **string lookup** (strawberry-django's default for a plain `choices` `CharField` — `exact` typed as the column scalar): echo the model's **stored** value (`draft`). The group key serializes to the enum member *name* (`DRAFT`), but the string lookup matches the stored value, so echoing the name would re-select nothing.
- **enum lookup** (the column exposed as a GraphQL enum input — a django-choices-field column, or an explicit `FilterLookup[StatusEnum]`): echo the enum **wire name** (`DRAFT`). A bare stored string would fail GraphQL enum-input validation.

The echo distinguishes the two by inspecting the lookup field's resolved strawberry type: an enum lookup carries a `StrawberryEnumDefinition` (from which `{stored_value: wire_name}` is read), a string lookup an unresolved type variable. Enum-typed → map the bucket's stored value to the filter enum's wire name; otherwise keep the stored value. Dates/times use ISO-8601; foreign-key ids fill strawberry-django's configured public-pk lookup (`pk` by default); `Decimal` and other non-JSON scalars fall back to their string form (matching how the scalar serializes).

For foreign-key axes, `filter_echo_relation_identity` may be passed to `AggregateBuilder` as a callable `(field, value) -> lookup_values`. `field` is the Django relation field and `value` is the grouped related database pk. The returned lookup mapping is validated against the live relation filter type exactly like every other echo lookup (a lookup name the filter does not expose is a fail-loud `FilterEchoError`, Rule 6). This lets a host framework emit `{customer: {sqid: "cus_..."}}` while the aggregate core stays permission- and identity-scheme-neutral.

The hook is **mandatory** whenever `DEFAULT_PK_FIELD_NAME` names an opaque public-id scheme: the library cannot tell a renamed-but-raw lookup (`id`) from an encoded one (`sqid`), so without the hook it echoes the raw pk under the configured name — a syntactically valid filter that re-selects no rows. Setting the hook without `enable_filter_echo=True` is a build-time `FilterEchoError` (the hook would never fire).
